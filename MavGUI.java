	...
	class CustomerPanel extends JPanel 
	{
...

		{
			...
			public void actionPerformed(ActionEvent event)
			{
				...
				if(valid())
				{	if (mav.customerExists(name) == false) 
				{
					c = new Customer(name, new Address(street, city, state, zip), ccNumber);
					mav.addCustomer(c);
				}

				else if (mav.customerExists(name) == true)
		...
		
		{
			...
			public void actionPerformed(ActionEvent event) 	
			{
				...
				{	
					...
					pricePerDay=Double.parseDouble(priceTextField.getText());if(furnitureCheckBox.isSelected())
				{
				...
				else if(electronicCheckBox.isSelected())
				{
					...
					if(mav.customerExists(name)==true)
				{
					
					for(Customer c:mav.getCustomerList())
					{
						if(c.getName().equals(name))
						{	
							if(furnitureCheckBox.isSelected())
								c.addRental(new FurnitureRental(pricePerDay,numberOfDays,FurnitureRental.FurnitureType.valueOf(type)));
							else if (electronicCheckBox.isSelected())
								c.addRental(new ElectronicRental(pricePerDay,numberOfDays,ElectronicRental.ElectronicType.valueOf(type)));
						}
					}
					
					}	
				else if(mav.customerExists(name)==false)
				...
				public boolean valid()
				{
					...
					if(!priceTextField.getText().isEmpty())
					{
						if(priceTextField.getText().matches("(\\d*\\.\\d{1}|\\d*\\.\\d{3,100})"))
						{
							JOptionPane.showMessageDialog(null, "Please round up to two decimal places");
							priceTextField.requestFocus();
							priceTextField.setText("");
							
							if(!priceTextField.getText().matches("\\d*\\.\\d\\d"))
						{
							JOptionPane.showMessageDialog(null, "Price is not the required type of double");
							priceTextField.requestFocus();
							priceTextField.setText("");
							validData=false;
						}
